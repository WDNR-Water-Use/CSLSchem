...
plot_HOBO_all <- function(lake, water_chem) {
  ltmp <- filter_parameter(water_chem, "TEMPERATURE HOBO") %>%
          arrange(.data$date) %>%
          filter(.data$lake == !!lake, !is.na(.data$result)) %>%
          mutate(day = floor_date(.data$date, unit = "day"))
  DO   <- filter_parameter(water_chem, "DISSOLVED OXYGEN HOBO") %>%
          filter(.data$lake == !!lake, !is.na(.data$result)) %>%
          mutate(day = floor_date(.data$date, unit = "day"))
          
  level <- CSLSdata::lake_levels
  level <- level %>% filter(.data$lake == !!lake)
  
  ltmp      <- merge(ltmp, level, by.x = "day", by.y = "date", all.x = TRUE)
  ltmp$elev <- ltmp$level_m - ltmp$depth1_m
  DO        <- merge(DO, level, by.x = "day", by.y = "date", all.x = TRUE)
  DO$elev   <- DO$level_m - DO$depth1_m

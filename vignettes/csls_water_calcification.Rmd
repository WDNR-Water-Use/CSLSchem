...
calcify$mass_kg <- calcify$calcification*calcify$mean_vol_m3

# Convert units involved in that calculation
mgTOmmol <- CSLSchem::mgTOmeq$mgTOmmol[mgTOmeq$description == "CALCIUM TOTAL RECOVERABLE"]
calcify$mass_kg <- calcify$mass_kg*1000/(12*mgTOmmol*1000*1000)
calcify  <- calcify %>%
...
calcify$site_type <- factor(calcify$site_type,
                            levels = c("P", "GWin", "GWout", "I-O", 
                                       "Delta Lake", "Cal"),
                            labels = c("P", "GWin", "GWout", "I-O", 
                                       expression(paste(Delta,"Lake")), 
                                       "Cal"))
...
```{r Calc03, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8.5, fig.height=5, fig.align="center"}
# Multiply lost calcium by lake volume
Ca_loss <- predictions %>%
           group_by(.data$lake) %>%
           summarise(loss_meqL = mean(.data$value)) %>%
           ungroup()
...
Ca_loss$mass_kg <- Ca_loss$loss_meqL*Ca_loss$mean_vol_m3

# Convert units involved in that calculation
mgTOmeq <- CSLSchem::mgTOmeq
mgTOmeq <- mgTOmeq$mgTOmeq[mgTOmeq$description == "CALCIUM TOTAL RECOVERABLE"]
Ca_loss$mass_kg <- Ca_loss$mass_kg*1000/(mgTOmeq*1000*1000)
